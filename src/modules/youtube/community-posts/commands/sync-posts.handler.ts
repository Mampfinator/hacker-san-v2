...
import { findActiveTab, parseRawData, CommunityPost, extractCommunityPosts, extractPost } from "yt-scraping-utilities";
import { findValuesByKeys } from "yt-scraping-utilities/dist/util";
import { YOUTUBE_BROWSE_ENDPOINT, YOUTUBE_CLIENT_VERSION } from "../../constants";
...
export class SyncPostsHandler implements ICommandHandler<SyncPostsCommand>, OnModuleInit {
    ...
    async onModuleInit() {
        ...
        while (!success) {
            await sleep(250);
            await page.click(".eom-buttons.style-scope.ytd-consent-bump-v2-lightbox>div>ytd-button-renderer")
            .then(() => {
                success = true;
            ...
            .catch(() => {
                success = false;
                this.logger.debug(`Failed getting new cookies from YouTube.`);
            });
        }
        ...
        this.logger.debug(`Got cookies from puppeteer: \n${cookies.map(cookie => cookie.toString()).join("\n")}`);

        await browser.close();
    }

    private getPostsAndToken(data: Record<string, any>): [CommunityPost[], string | undefined] {
        const posts: CommunityPost[] = findValuesByKeys(data, ["sharedPostRenderer", "backstagePostRenderer"]).map(extractPost);

        const [continuationRenderer] = findValuesByKeys(data, ["continuationItemRenderer"]);
        const token = continuationRenderer?.continuationEndpoint?.continuationCommand?.token;
    ...
    async execute({ channelId, posts }: SyncPostsCommand) {
        ...
        if (channelId && !posts) {
            ...
            while (!communityTab) {
                try {
                    const data = await this.youtubeService.fetchRaw(communityUrl, {useInstance: client, requestOptions: {headers}}, false) as string;
                    ytInitialData = parseRawData({source: data, ytInitialData: true}).ytInitialData;
            ...
            clickTrackingParams = communityTab.tabRenderer.content.sectionListRenderer.trackingParams;
            const visitorData = ytInitialData.responseContext.webResponseContextExtensionData.ytConfigData.visitorData;

            let [allPosts, continuationToken] = this.getPostsAndToken(ytInitialData);
            ...
            Object.assign(headers, {
                ...
                Cookies: (await this.cookies.getCookies("https://youtube.com")).map(cookie => cookie.toString()).join("; ")
            });

            // TODO: test if this is even necessary?
            const SAPISID = (await this.cookies.getCookies(communityUrl)).find(cookie => cookie.key === "SASIPID");
            if (SAPISID) {
                const time = `${Math.round(new Date().getTime()/1000)}`;
                Object.assign(headers, {
                    Authorization: `SAPISIDHASH ${time}_${createHash("sha1").update(Buffer.from(time)).update(Buffer.from(SAPISID.value)).update(Buffer.from("https://www.youtube.com")).digest("hex")}`
                });
            }
            ...
            while (continuationToken) {
                ...
                const data = await this.youtubeService.fetchRaw(YOUTUBE_BROWSE_ENDPOINT, {requestOptions: {data: body, headers, method: "POST"}, useInstance: client}, false) as Record<string, any>;
                const [newPosts, newToken] = this.getPostsAndToken(data);
                allPosts.push(...newPosts);

                headers.clickTrackingParams = data.onResponseReceivedEndpoints[0].clickTrackingParams;
                continuationToken = newToken;
            }

            posts = allPosts;
            this.logger.debug(`Found a total of ${posts.length} posts for ${channelId}.`);
        }
