...
export class FetchPostHandler implements ICommandHandler<FetchPostCommand> {
    ...
    async execute({ postId, forceRefetch, includeChannel }: FetchPostCommand): Promise<any> {
        if (!forceRefetch && this.cache.has(postId)) return this.cache.get(postId);
        const result = await this.requestService.fetchSinglePost(postId, includeChannel);
