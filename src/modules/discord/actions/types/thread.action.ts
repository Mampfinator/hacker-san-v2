...
export class ThreadAction implements IActionType {
    constructor(
        ...
        @InjectRepository(StreamDiscordChannelMap) private readonly streamChannelMaps: Repository<StreamDiscordChannelMap>
    ) {}
        ...
        if (mode === "create") {
            ...
            switch(channel.type) {
                case ChannelType.GuildCategory:
                    subchannel = await channel.children.create({name: interpolate(name ?? DEFAULT_NAME, {descriptor, payload}), type: ChannelType.GuildText});
                    await subchannel.send(interpolate(message ?? DEFAULT_MESSAGE, {descriptor, payload}));
                    break;
                case ChannelType.GuildText:
                    subchannel = await channel.threads.create({name: interpolate(name ?? DEFAULT_NAME, {descriptor, payload}), startMessage: interpolate(message ?? DEFAULT_MESSAGE, {descriptor, payload}), type: ChannelType.PublicThread}) as PublicThreadChannel;
                    break;
                case ChannelType.GuildForum:
                    subchannel = await channel.threads.create({name: interpolate(name ?? DEFAULT_NAME, {descriptor, payload}), message: {content: interpolate(message ?? DEFAULT_MESSAGE, {descriptor, payload})}}) as PublicThreadChannel;
                    break;
            }
            ...
            const channels = await this.streamChannelMaps.find({where: {
                platform,
                platformId,
            }});
        }
