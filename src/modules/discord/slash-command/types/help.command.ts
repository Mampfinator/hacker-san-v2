...
export class HelpCommand {
    @Command()
    handle(
        @String({name: "topic", description: "What exactly you need help on. Omit for an overview.", required: false}) topic?: string
    ) {
        return {embeds: [
            new EmbedBuilder()
                .setColor(Colors.Green)
                .setDescription(`You searched for: ${topic ?? "Nothing!"}`)
        ]};
    }
}